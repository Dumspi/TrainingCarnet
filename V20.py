...
ATHLETE_GROUPS = {
    ...
    "Marie": "marteau poids disque",
    "Dorine": "marteau poids disque",
    "Yara": "marteau poids disque",
    "Julie": "marteau poids disque",
}
...
def get_profile_codes():
    ...
    try:
        ...
        st.error(
            ...
            "Crée le fichier .streamlit/secrets.toml en local, ou ajoute "
            "les mêmes secrets dans Streamlit Community Cloud."
        )
        st.stop()
    ...
    missing = [
        ...
        if normaliser_texte(athlete) not in codes
    ]
...
def verifier_code(athlete, entered_code):
    expected_code = get_profile_codes()[
        normaliser_texte(athlete)
    ]
    return hmac.compare_digest(
        str(entered_code).strip(),
        expected_code,
    )
...
def afficher_connexion():
    ...
    st.write(
        "Sélectionne ton profil puis saisis ton code personnel."
    )
    ...
    if submitted:
        if verifier_code(selected_athlete, entered_code):
...
st.set_page_config(
    page_title="Carnet Team Lancers — V20",
    layout="centered",
)

st.sidebar.title("👤 Mon profil")
st.sidebar.success(athlete)
